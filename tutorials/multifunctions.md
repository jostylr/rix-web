...
Prerequisites: [Define and call](function-basics.html) and the first two sections of [Ternaries and cases](ternaries.html). A prepared guard chooses which implementation applies; use an ordinary ternary when one function is simply choosing a result.

Run the simple absolute-value variants first, then compare how later guards accept, reject, or leave a case undecided.
...
Change the input from negative to positive and check which variant runs before exploring the advanced guard forms.
...
Continue with [Rule dispatcher](capstone-dispatch.html) for a complete policy task.
