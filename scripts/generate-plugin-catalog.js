import path from "node:path";
import { NodePluginCatalog } from "../../rix/src/runtime/plugin-catalog-node.js";

const root = path.resolve(import.meta.dir, "..");
// Keep the browser's approved plugins explicit. The package source lives in
// RiX itself; this generator turns only these reviewed packages into imports.
const pluginRoots = [
    path.resolve(root, "..", "rix", "plugins", "float"),
    path.resolve(root, "..", "rix", "plugins", "ball"),
    path.resolve(root, "..", "rix", "plugins", "cauchy"),
    path.resolve(root, "..", "rix", "plugins", "continued-fraction"),
    path.resolve(root, "..", "rix", "plugins", "algebraic-real"),
    path.resolve(root, "..", "rix", "plugins", "complex"),
    path.resolve(root, "..", "rix", "plugins", "exact-algebras"),
    path.resolve(root, "..", "rix", "plugins", "quaternion"),
    path.resolve(root, "..", "rix", "plugins", "octonion"),
    path.resolve(root, "..", "rix", "plugins", "cayley"),
    path.resolve(root, "..", "rix", "plugins", "numerics"),
    path.resolve(root, "..", "rix", "plugins", "bessel"),
    path.resolve(root, "..", "rix", "plugins", "oracle"),
    path.resolve(root, "..", "rix", "plugins", "radix"),
    path.resolve(root, "..", "rix", "plugins", "fraction"),
    path.resolve(root, "..", "rix", "plugins", "stern-brocot"),
    path.resolve(root, "..", "rix", "plugins", "fracfun"),
    path.resolve(root, "..", "rix", "plugins", "poly"),
    path.resolve(root, "..", "rix", "plugins", "ratfun"),
    path.resolve(root, "..", "rix", "plugins", "symbolic"),
    path.resolve(root, "..", "rix", "plugins", "cas"),
    path.resolve(root, "..", "rix", "plugins", "calculus"),
    path.resolve(root, "..", "rix", "plugins", "ode"),
    path.resolve(root, "..", "rix", "plugins", "analysis"),
    path.resolve(root, "..", "rix", "plugins", "algebra"),
    path.resolve(root, "..", "rix", "plugins", "logic"),
    path.resolve(root, "..", "rix", "plugins", "combinatorics"),
    path.resolve(root, "..", "rix", "plugins", "graph"),
    path.resolve(root, "..", "rix", "plugins", "draw"),
    path.resolve(root, "..", "rix", "plugins", "plot"),
    path.resolve(root, "..", "rix", "plugins", "scene3d"),
    path.resolve(root, "..", "rix", "plugins", "nd"),
    path.resolve(root, "..", "rix", "plugins", "geometry"),
    path.resolve(root, "..", "rix", "plugins", "data"),
    path.resolve(root, "..", "rix", "plugins", "linalg"),
    path.resolve(root, "..", "rix", "plugins", "optimize"),
    path.resolve(root, "..", "rix", "plugins", "solve"),
    path.resolve(root, "..", "rix", "plugins", "stats"),
    path.resolve(root, "..", "rix", "plugins", "probability"),
    path.resolve(root, "..", "rix", "plugins", "document"),
    path.resolve(root, "..", "rix", "plugins", "complex-visualization"),
    path.resolve(root, "..", "rix", "plugins", "fractals"),
    path.resolve(root, "..", "rix", "plugins", "render-terminal-ascii"),
    path.resolve(root, "..", "rix", "plugins", "render-svg"),
    path.resolve(root, "..", "rix", "plugins", "render-canvas"),
    path.resolve(root, "..", "rix", "plugins", "render-webgl"),
    path.resolve(root, "..", "rix", "plugins", "render-tikz"),
    path.resolve(root, "..", "rix", "plugins", "render-png"),
    path.resolve(root, "..", "rix", "plugins", "render-markdown"),
    path.resolve(root, "..", "rix", "plugins", "render-html"),
    path.resolve(root, "..", "rix", "plugins", "render-quarto"),
    path.resolve(root, "..", "rix", "plugins", "render-latex"),
    path.resolve(root, "..", "rix", "plugins", "render-pdf"),
    path.resolve(root, "..", "rix", "plugins", "render-gltf"),
    path.resolve(root, "..", "rix", "plugins", "render-csv"),
    path.resolve(root, "..", "rix", "plugins", "render-gif"),
    path.resolve(root, "..", "rix", "examples", "plugins", "example-array-js"),
    path.resolve(root, "..", "rix", "examples", "plugins", "example-array-rix"),
];
const outputPath = path.join(root, "src", "generated", "bundled-plugin-catalog.js");
const catalog = new NodePluginCatalog({ roots: pluginRoots }).scan();
const browserInstaller = {
    float: path.resolve(root, "..", "rix", "plugins", "float", "browser-installer.js"),
};
const relativeImport = (from, to) => {
    const relative = path.relative(path.dirname(from), to).split(path.sep).join("/");
    return relative.startsWith(".") ? relative : `./${relative}`;
};

const hostEntries = catalog.list().filter((entry) => entry.kind === "host");
const imports = hostEntries.map((entry, index) => (
    `import { install as install${index} } from ${JSON.stringify(relativeImport(outputPath, browserInstaller[entry.id] || entry.sourcePath))};`
));
const registrations = catalog.list().map((entry) => {
    const metadata = { ...entry };
    delete metadata.source;
    // The generated browser bundle must not retain the builder's absolute
    // filesystem paths. They are only diagnostic labels once embedded.
    metadata.sourcePath = `bundled:${entry.id}`;
    if (entry.kind === "rix") {
        return `  catalog.addMetadata(${JSON.stringify(metadata)}, { source: ${JSON.stringify(entry.source)}, sourcePath: ${JSON.stringify(metadata.sourcePath)}, kind: "rix" });`;
    }
    const installer = hostEntries.findIndex((candidate) => candidate.id === entry.id);
    return `  catalog.addMetadata(${JSON.stringify(metadata)}, { sourcePath: ${JSON.stringify(metadata.sourcePath)}, kind: "host" });\n  catalog.registerInstaller(${JSON.stringify(entry.id)}, install${installer});`;
});

const generated = `// Generated by scripts/generate-plugin-catalog.js. Do not edit manually.\nimport { PluginCatalog } from "../../../rix/src/index.js";\n${imports.join("\n")}\n\nexport function createBundledPluginCatalog() {\n  const catalog = new PluginCatalog();\n${registrations.join("\n")}\n  return catalog;\n}\n`;
await Bun.write(outputPath, generated);
