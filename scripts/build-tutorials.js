...
function renderInline(text) {
    ...
    html = html.replace(/\[([^\]]+)\]\(((?:\.\/)?[A-Za-z0-9][A-Za-z0-9.-]*\.html(?:#[A-Za-z0-9_-]+)?)\)/g,
        (_, label, target) => `<a href="${target.startsWith("./") ? target : `./${target}`}">${label}</a>`);
