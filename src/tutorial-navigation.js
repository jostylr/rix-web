...
export function renderTutorialSidebar(document, container, tutorials, currentFile) {
    ...
    for (const root of rootsOf(tutorials)) {
        ...
        if (!children.length) {
            ...
            if (current?.number === root.number) anchor.classList.add("current");
        ...
        if (current?.number === root.number) overview.classList.add("current");
        details.append(overview);
        for (const child of children) {
            const anchor = link(document, child);
            if (current?.number === child.number) anchor.classList.add("current");
...
export function renderTutorialPageNavigation(document, container, tutorials, currentFile) {
    ...
    const section = current.parent
        ? tutorials.find((tutorial) => tutorial.number === current.parent)
        : current;
...
function renderIndexSection(document, tutorial, tutorials) {
    ...
    if (children.length) {
        ...
        for (const child of children) childContainer.append(link(document, child));
        section.append(childContainer);
    }
    return section;
}

export function renderTutorialIndex(document, container, tutorials, pluginGroup) {
...
export async function mountTutorialNavigation({ document = globalThis.document, fetchImpl = globalThis.fetch } = {}) {
    if (!document) return;
    ...
    try {
        ...
        if (!response.ok) throw new Error(`Tutorial navigation request failed (${response.status})`);
