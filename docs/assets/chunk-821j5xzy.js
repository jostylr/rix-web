...
function combineDimensions(left, right, sign = 1) {
  ...
  for (const [name, exponent] of Object.entries(right)) {
    ...
    if (next === 0)
      ...
      dimensions[name] = next;
  }
  return dimensions;
}
function scaleDimensions(dimensions, exponent) {
  return Object.fromEntries(Object.entries(dimensions).map(([name, power]) => [name, power * exponent]).filter(([, power]) => power !== 0));
}
...
function factorsFromUnit(unit) {
  if (unit.type === "unit_expr")
    return new Map(unit.factors);
...
function ensureLinearUnit(unit) {
  if (unit?.affine)
    throw new Error(`Affine unit '${unit.symbol}' cannot be used in a compound unit expression`);
}
function multiplyUnits(left, right) {
  ...
  for (const [factor, exponent] of factorsFromUnit(right)) {
    ...
    if (next === 0)
      ...
      factors.set(factor, next);
  }
  return createUnitExpr(combineDimensions(left.dimensions, right.dimensions), multiplyScalars(left.scale, right.scale), factors);
}
function divideUnits(left, right) {
  ...
  return createUnitExpr(combineDimensions(left.dimensions, right.dimensions, -1), divideScalars(left.scale, right.scale), factors);
}
...
function powUnit(unit, exponentValue) {
  ensureLinearUnit(unit);
...
function constructQuantity(magnitude, unit) {
  if (!isScalar(magnitude))
    throw new Error("A unit must be applied to an exact scalar value");
  if (!isUnitValue(unit))
    throw new Error("Quantity construction requires a Unit or UnitExpr");
...
function displayMagnitude(quantity) {
...
function convertQuantity(quantity, target) {
  if (!isQuantity(quantity))
    throw new Error("ConvertUnit expects a quantity");
  if (!isUnitValue(target))
    throw new Error("ConvertUnit target must be a Unit or UnitExpr");
...
function subtractQuantities(left, right) {
  if (!dimensionsEqual(left.dimensions, right.dimensions)) {
  ...
  const displayUnit = pointDifference ? left.displayUnit.differenceUnit || createUnit(`delta${left.displayUnit.symbol}`, {
...
function multiplyQuantityValues(left, right) {
  if (isQuantity(left) && isQuantity(right)) {
    if (left.affinePoint || right.affinePoint)
      throw new Error("Affine quantity points cannot be multiplied");
  ...
  if (!isScalar(scalar))
    throw new Error("Quantity multiplication requires a scalar or quantity");
  return { ...quantity, baseMagnitude: multiplyScalars(quantity.baseMagnitude, scalar) };
}
function divideQuantityValues(left, right) {
  if (isQuantity(left) && isQuantity(right)) {
    if (left.affinePoint || right.affinePoint)
      throw new Error("Affine quantity points cannot be divided");
  ...
  if (isScalar(left) && isQuantity(right)) {
    if (right.affinePoint)
      throw new Error("Cannot divide by an affine quantity point");
    return {
      type: "quantity",
...
function powQuantity(quantity, exponentValue) {
  if (quantity.affinePoint)
...
function compareQuantities(left, right) {
  ...
  if (isExactValue(difference))
    throw new Error("Exact symbolic quantity expressions are not ordered");
  if (difference instanceof Integer)
    return difference.value < 0n ? -1 : difference.value > 0n ? 1 : 0;
  if (difference instanceof Rational)
...
function formatQuantity(quantity, formatScalar) {
...
function parseUnitExpression(source, collection) {
  ...
  function parsePrimary() {
    if (input[index] === "(") {
      ...
      if (input[index] !== ")")
        throw new Error(`Expected ')' in unit expression '${source}'`);
      index++;
      return value;
    }
    const match = /^[\p{L}_][\p{L}\p{N}_]*/u.exec(input.slice(index));
    if (!match)
      throw new Error(`Invalid unit expression '${source}' at character ${index + 1}`);
  ...
  if (index !== input.length)
    throw new Error(`Invalid trailing input in unit expression '${source}'`);
  return result;
}
...
function createDefaultUnitCollection() {
  const entries = new Map;
  ...
  add("degF", {
    ...
    affine: true,
    scale: rat(5n, 9n),
    offset: rat(45967n, 180n),
    differenceUnit: deltaDegF
  }, ["fahrenheit"]);
  return {
    type: "map",
    entries,
...
function mapLazySequence(source, mapper, options = {}) {
  let mapped;
...
function filterLazySequence(source, predicate, options = {}) {
  let filtered;
...
function transformPlan(directionValue, inlineArgs) {
  if (directionValue === null || directionValue === undefined) {
    if (inlineArgs.length)
      throw new Error("Transform arguments require a direction");
    return [{ direction: "identities", args: [] }];
  }
  if (directionValue?.type === "tuple") {
    if (inlineArgs.length)
      throw new Error("A Transform tuple carries each operation's arguments inside its operation array");
...
function exactCenter(value) {
  ...
  if (!isExactScalar(value))
    throw new Error("Center transformation requires an exact integer or rational center");
...
function polynomialDegree(polynomial) {
  let degree = null;
  for (const power of polynomial.keys())
    if (degree === null || power > degree)
      degree = power;
  return degree;
}
function polynomialDivide(dividend, divisor) {
  ...
  while (true) {
    ...
    if (remainderDegree === null || remainderDegree < divisorDegree)
      break;
...
function decompositionOperand(value, variable) {
  if (isExactScalar(value)) {
    ...
    return {
      ...
      polynomial: polynomialFromIr(expression2, variable, new Map([[1n, literal(1)]])),
      closureScopes: []
    };
  }
  const spec = getAttachedSpec(value);
...
function transformValue(args) {
  ...
  const source = getAttachedSpec(value);
  if (!source)
    throw new Error("Transform expects a symbolic spec or function with an attached spec");
  ...
  for (const operation of plan) {
    if (["identities", "constants", "powers", "expand"].includes(operation.direction)) {
      if (operation.args.length)
        throw new Error(`Transform direction '${operation.direction}' does not accept arguments`);
      const directions = new Set(["identities", "constants", "powers"]);
      if (operation.direction === "expand")
        directions.add("expand");
...
function registerBuiltinSemanticTypes() {
  ...
  const nativeOnly = [
    ...
    ["Array", "array", ["sequence"], (value) => value?.type === "sequence" || value?.type === "lazy_sequence" ? value : null],
  ...
  registerType({
    ...
    export(value) {
      const parts = rationalParts2(value);
      return {
        type: "map",
        entries: new Map([
    ...
    installs: {
      ADD: [{
        name: "RatRat",
        prep: (args) => args.length === 2 && rationalParts2(args[0]) && rationalParts2(args[1]),
        impl: ([a, b]) => a.add(b)
      }],
      SUB: [{
        name: "RatRat",
...
function descriptorValue(metadata) {
  ...
  for (const name of metadata.exports) {
    ...
    entries.set(displayName, method2);
    extension.set(displayName.toUpperCase(), method2);
  }
  return { type: "map", entries, _ext: extension };
}
...
class PluginCatalog {
  ...
  registerInstaller(id, installer) {
    if (typeof installer !== "function")
      throw new Error(`Plugin installer '${id}' must be a function`);
  ...
  addMetadata(metadata, { sourcePath = `<plugin:${metadata?.id || "unknown"}>`, source = null, kind = metadata?.kind } = {}) {
    ...
    const complete2 = { ...entry, source };
    if (this.entries.has(complete2.id))
      throw new Error(`Duplicate plugin id '${complete2.id}'`);
    this.entries.set(complete2.id, complete2);
    return complete2;
  }
  list() {
...
function sliceSequenceLike(obj, spec2) {
  if (isLazySequence(obj)) {
    ...
    for (let i = startRaw;step > 0 ? i <= endRaw : i >= endRaw; i += step) {
      if (i <= obj._lazy.cache.length)
        out.push(obj._lazy.cache[i - 1]);
    }
...
function indexGetResolved(obj, key) {
  ...
  if (isLazySequence(obj)) {
    ...
    if (idx < 0)
      return indexGetResolved(materializeLazySequence(obj), key);
    if (idx === 0)
      throw new Error("Sequence indexes are 1-based; zero is invalid");
...
function toBaseString(value, baseSystem, modeSpec = { mode: 1 }) {
  ...
  if (mode === 3 || mode === 4) {
    const cf = rat2.toContinuedFraction();
  ...
  if (mode === 6) {
    return `${toBaseDigitsInt(rat2.numerator, baseSystem)}/${toBaseDigitsInt(rat2.denominator, baseSystem)}`;
  }
  if (rat2.denominator === 1n) {
    return toBaseDigitsInt(rat2.numerator, baseSystem);
  }
  const sign = rat2.numerator < 0n ? -1n : 1n;
  const absNum = rat2.numerator < 0n ? -rat2.numerator : rat2.numerator;
  const whole = absNum / rat2.denominator;
  const rem = absNum % rat2.denominator;
  if (rem === 0n)
    return toBaseDigitsInt(rat2.numerator, baseSystem);
...
function parseLiteral(str) {
  ...
  const posStr = isNegative2 ? str.slice(1) : str;
  ...
  if (posStr.includes("_^")) {
    ...
    if (shiftMatch) {
      const baseVal = parseLiteral((isNegative2 ? "-" : "") + shiftMatch[1]);
  ...
  if (customMatch) {
    ...
    valueStr = (isNegative2 ? "-" : "") + customMatch[2];
  } else {
    ...
    if (prefixMatch) {
      ...
      if (baseSystem) {
        valueStr = (isNegative2 ? "-" : "") + prefixMatch[2];
      }
    }
  }
  if (baseSystem) {
...
function toRational(val, name = "value") {
  if (val instanceof Rational)
    return val;
  if (val instanceof Integer)
    return new Rational(val.value, 1n);
...
function finiteLazySequence(length, valueAt, label, maxIterations = 1e4) {
...
function simplestRandomRational(lo, hi, tolerance, context) {
  ...
  const tol = tolerance ?? context?.getEnv?.("randomTolerance", 0.000001) ?? 0.000001;
  if (!(tol > 0))
    throw new Error("Random rational tolerance must be positive");
  const lower2 = Math.max(loNumber, x - tol);
  const upper = Math.min(hiNumber, x + tol);
  const maxDenominator = context?.getEnv?.("randomMaxDenominator", 1e6) ?? 1e6;
  for (let d = 1;d <= maxDenominator; d++) {
    const min = Math.ceil(lower2 * d - Number.EPSILON);
...
function randomParameters(value) {
  ...
  const count = toInteger2(values[0], "Random sample count");
  if (count <= 0)
    throw new Error("Random sample count must be positive");
  let denominator = null;
  if (values[1] !== null && values[1] !== undefined) {
    if (values[1] instanceof Integer)
      denominator = values[1].value;
    else if (values[1] instanceof Rational && values[1].denominator === 1n)
      denominator = values[1].numerator;
    else if (typeof values[1] === "bigint")
      denominator = values[1];
    else {
      const parsed = toInteger2(values[1], "Random denominator");
...
var advancedFunctions = {
  ...
  STEP: {
    impl(args, context) {
      ...
      return attachBuiltinProto(createLazySequence({
        ...
        cloneState: (state) => ({ ...state }),
        knownLength,
        maxIterations: context?.getEnv?.("generatorMaxIterations", 1e4) ?? 1e4,
        label: "stepped interval",
        pull(state) {
          const value = state.started ? state.current.add(step) : state.current;
          state.started = true;
          state.current = value;
          if (direction > 0 && value.compareTo(bound) > 0 || direction < 0 && value.compareTo(bound) < 0)
            return { done: true };
          return { done: false, value, attempts: 1 };
        }
      }));
  ...
  DIVIDE: {
    impl(args, context) {
      const [lo, hi] = intervalBounds(args[0]);
      const count = toInteger2(args[1], "Point count");
      if (count <= 0)
        throw new Error("Point count must be positive");
      if (count === 1)
        return finiteLazySequence(1, () => lo, "interval division", context?.getEnv?.("generatorMaxIterations", 1e4));
      const step = hi.subtract(lo).divide(new Rational(BigInt(count - 1), 1n));
      return finiteLazySequence(count, (index) => lo.add(step.multiply(new Rational(BigInt(index), 1n))), "interval division", context?.getEnv?.("generatorMaxIterations", 1e4));
    },
    pure: true,
  ...
  PARTITION: {
    impl(args) {
      const [lo, hi] = intervalBounds(args[0]);
      const count = toInteger2(args[1], "Partition count");
      if (count <= 0)
        throw new Error("Partition count must be positive");
  ...
  MEDIANTS: {
    impl(args) {
      const [lo, hi] = intervalBounds(args[0]);
      const levels = toInteger2(args[1], "Mediant level count");
      if (levels < 0)
        throw new Error("Mediant level count cannot be negative");
  ...
  RANDOM: {
    impl(args, context) {
      ...
      const values = Array.from({ length: count }, () => denominator === null ? simplestRandomRational(lo, hi, tolerance, context) : randomGridRational(lo, hi, denominator, context));
  ...
  RANDOM_PARTITION: {
    impl(args, context) {
      ...
      const { count, denominator, tolerance } = randomParameters(args[1]);
      if (count === 1)
        return eagerSequence([new RationalInterval(lo, hi)]);
      const points = new Map;
      const maxAttempts = context?.getEnv?.("generatorMaxIterations", 1e4) ?? 1e4;
      let attempts = 0;
      while (points.size < count - 1 && attempts++ < maxAttempts) {
        const point = denominator === null ? simplestRandomRational(lo, hi, tolerance, context) : randomGridRational(lo, hi, denominator, context);
        if (point.compareTo(lo) > 0 && point.compareTo(hi) < 0)
          points.set(point.toString(), point);
      }
      if (points.size < count - 1)
        throw new Error("Could not choose enough distinct interior random partition points");
...
function parseExactExpression(source, collection) {
  ...
  function primary() {
    if (input[index] === "(") {
      ...
      const value = product();
      if (input[index] !== ")")
        throw new Error(`Expected ')' in exact expression '${source}'`);
      index++;
      return value;
    }
    const match = /^[\p{L}_][\p{L}\p{N}_]*/u.exec(input.slice(index));
  ...
  if (!input)
    throw new Error("Exact expression cannot be empty");
  const result = product();
  if (index !== input.length)
    throw new Error(`Invalid trailing input in exact expression '${source}'`);
  return result;
}
function multiplyWithUnits(left, right) {
  ...
  if (isUnitValue(left) && isScalar(right))
    return constructQuantity(right, left);
...
function installUnitExactVariants(registry) {
  const cayleyArgs = (args) => args.length === 2 && args.some(isCayleyValue) && args.every((value) => isCayleyValue(value) || isScalar(value));
  const cayleyVariants = {
    ...
    EQ: ([a, b]) => boolResult3(equalCayley(a, b)),
    NEQ: ([a, b]) => boolResult3(!equalCayley(a, b))
  };
  for (const [name, impl] of Object.entries(cayleyVariants)) {
    registry.installVariant(name, {
      name: `Cayley_${name}`,
      prep: (args) => name === "POW" ? args.length === 2 && isCayleyValue(args[0]) : cayleyArgs(args),
      impl
    });
  }
  registry.installVariant("NEG", {
  ...
  const exactVariants = {
    ...
    EQ: ([a, b]) => boolResult3(equalScalars(a, b)),
    NEQ: ([a, b]) => boolResult3(!equalScalars(a, b))
  };
  ...
  const unitVariants = {
    ...
    EQ: ([a, b]) => boolResult3(isQuantity(a) && isQuantity(b) ? quantitiesEqual(a, b) : unitsEquivalent(a, b)),
    NEQ: ([a, b]) => boolResult3(isQuantity(a) && isQuantity(b) ? !quantitiesEqual(a, b) : !unitsEquivalent(a, b)),
    LT: ([a, b]) => boolResult3(compareQuantities(a, b) < 0),
    GT: ([a, b]) => boolResult3(compareQuantities(a, b) > 0),
    LTE: ([a, b]) => boolResult3(compareQuantities(a, b) <= 0),
    GTE: ([a, b]) => boolResult3(compareQuantities(a, b) >= 0)
  };
  for (const [name, impl] of Object.entries(unitVariants)) {
    registry.installVariant(name, {
      ...
      prep: (args) => {
        if (name === "POW")
          return args.length === 2 && (isUnitValue(args[0]) || isQuantity(args[0]));
